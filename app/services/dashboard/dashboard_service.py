...
``REPORT_VIEW`` pour tout ce qui provient de ``ReportService``) — un rôle
disposant de ``DASHBOARD_VIEW`` sans disposer de la permission source ne
voit simplement pas la section correspondante (comportement déjà identique
à la navigation filtrée de ``MainWindow``), jamais une erreur qui casserait
la page. Licence : ``DASHBOARD_VIEW`` est mappée sur la fonctionnalité
``REPORTS`` déjà existante (voir ``permission_map.py``) — aucun nouveau code
de licence n'est introduit.
"""
...
class DashboardOverview:
    ...
    activity: Optional[ActivityKpis]
...
class DashboardService:
    def __init__(
        self, permission_service: PermissionService, report_service: ReportService, settings: Optional[Settings] = None
    ) -> None:
    ...
    def get_activity_kpis(self, period_from: date, period_to: date) -> Optional[ActivityKpis]:
        self._require_dashboard()
        if not self._has_report_access():
            return None
        ...
        sales = self._reports.get_sales(date_from=period_from, date_to=period_to, statut=StatutOperation.VALIDEE)
        inventories_validated = sum(
            1 for i in self._reports.get_inventories(date_from=period_from, date_to=period_to) if i.statut.value == "VALIDE"
        )
        sales_amount = sum((v.total for v in sales), Decimal("0"))

        return ActivityKpis(
            period_from=period_from, period_to=period_to,
            entries_validated=len(entries), exits_validated=len(exits),
            sales_validated=len(sales), sales_amount=sales_amount,
