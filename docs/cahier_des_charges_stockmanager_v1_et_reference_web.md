...
- **Mode d'activation LOCAL/SERVER** (`app/services/licensing/activation_mode.py`, `activation_service.py`, `license_server_client.py`) : préparation d'architecture pour un futur serveur TechNova, sans dépendance à ce serveur aujourd'hui. `ActivationMode.LOCAL` (par défaut) et `ActivationMode.SERVER`, persisté par installation dans `parametres` (clé `activation.mode`, valeur absente/invalide → toujours `LOCAL` par sécurité). `ActivationService` orchestre le choix de mode SANS remplacer `LicenseService`, qui reste l'unique autorité de validation cryptographique et de stockage : en mode LOCAL, `ActivationService.activate()` délègue intégralement à `LicenseService.activate_license()` (comportement strictement inchangé) ; en mode SERVER, la licence est validée localement (`LicenseService.validate_license_content()`, sans persistance) puis soumise à un `LicenseServerClient` — **aucune implémentation réseau réelle n'existe à ce stade** (`UnconfiguredLicenseServerClient` lève systématiquement `LicenseServerUnavailableError`), une tentative SERVER ne peut donc jamais être confondue avec une activation réussie. `FeatureGate` n'a subi aucune modification : il continue de consulter exclusivement `LicenseService.evaluate()`, le mode d'activation ne concernant que le processus d'activation, jamais l'évaluation quotidienne (100 % hors ligne dans les deux modes).

### B. ÉVOLUTION POSSIBLE (WEB)
Une architecture SaaS nécessiterait une **gestion centralisée des licences** côté serveur (activation, renouvellement, révocation à distance, application réelle de `max_devices`/`max_postes` à travers une flotte de postes) — voir §37.10. Le mode SERVER préparé ci-dessus (§A) est le point d'ancrage prévu pour cette évolution : le jour où un serveur TechNova réel existera, seule une nouvelle implémentation de `LicenseServerClient` sera nécessaire, sans modification d'`ActivationService` ni de `LicenseService`.
